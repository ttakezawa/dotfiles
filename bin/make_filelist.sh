...
locate --regex "^$HOME" | grep -Pv $ignore_dir | grep -Pv $ignore >> $home_filelist.new
...
locate --regex '^/(bin|boot|etc|lib|opt|sbin|usr|var)' | grep -Pv $ignore_dir | grep -Pv $ignore >> $system_filelist.new
